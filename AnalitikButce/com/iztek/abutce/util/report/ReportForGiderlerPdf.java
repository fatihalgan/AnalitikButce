...
public class ReportForGiderlerPdf implements ReportInterface {
	private RaporBean raporBean = null;
	private DefaultMutableTreeNode root = null;
	private String fileOutPath = "ButceRaporu.pdf";
	
	public ReportForGiderlerPdf(RaporBean raporBean, DefaultMutableTreeNode root) {
	...
	public void getReport() {
		...
		try {
			...
			int headerwidths[] = {3,3,3,3,3,3,3,3,5,3,3,3,3,29,10,10,10};
			table.setWidths(headerwidths);
			table.setWidth(100);

			Cell topCell = new Cell(new Phrase(raporBean.getYil()+" YILI BÝRLEÞÝK ÖDENEK CETVELÝ", fontLarge));
			topCell.setBorder(Rectangle.NO_BORDER);
			topCell.setColspan(17);
			...
			cell.setRowspan(2);
			table.addCell(cell);
			cell = new Cell(new Phrase(raporBean.getYil()+" YILI BÜTÇESÝ SAYMANIN TEKLÝFÝ", fontSmall));
			cell.setRowspan(2);
			table.addCell(cell);
			cell = new Cell(new Phrase("BELEDÝYE BAÞKANINCA UYGUN GÖRÜLEN", fontSmall));
			cell.setRowspan(2);
			table.addCell(cell);
			cell = new Cell(new Phrase("ENCÜMENCE KABUL EDiLEN", fontSmall));
			cell.setRowspan(2);
			table.addCell(cell);
			...
			if(root != null) {
				...
				while(enum.hasMoreElements()) {
					...
					if(raporBean.isSayman()) {
						cell = new Cell(new Phrase(bean.getOdenekSayman().convertTo(GuiController.getInstance().getDefaultUnit()).toString(), fontNormal));
					}else {
					...
					if(raporBean.isBaskan()) {
						cell = new Cell(new Phrase(bean.getOdenekBaskan().convertTo(GuiController.getInstance().getDefaultUnit()).toString(), fontNormal));
					}else {
						cell = new Cell(new Phrase("", fontNormal));
					}
					cell.setBorder(Rectangle.RIGHT);
					...
					if(raporBean.isEncumen()) {
						cell = new Cell(new Phrase(bean.getOdenekEncumen().convertTo(GuiController.getInstance().getDefaultUnit()).toString(), fontNormal));
					}else {
						cell = new Cell(new Phrase("", fontNormal));
					}
					cell.setBorder(Rectangle.RIGHT);
